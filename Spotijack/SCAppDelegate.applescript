...
script SCAppDelegate
  ...
  property NSProcessInfo : class "NSProcessInfo"
  ...
  property activityToken : missing value
  ...
  on applicationShouldTerminate_(sender)
    if my playingMusic as boolean is false
    resetApps_(false)
    return current application's NSTerminateNow
    else
    set answer to button returned of (display dialog "Currently recording. Quit & cancel the recording?" buttons {"No", "Yes"} default button "No")
...
on recordMusic_()
  # Tell the system we're about to do something and don't want to sleep
  set process to NSProcessInfo's processInfo()
  set my activityToken to process's beginActivityWithOptions_reason_(current application's NSActivityIdleSystemSleepDisabled, "stuff")
  ...
  process's endActivity_(my activityToken)
end recordMusic_
...
on resetApps_(pauseSpotify)
  ...
  if pauseSpotify as boolean is true then tell application "Spotify" to pause current track
end resetApps_
